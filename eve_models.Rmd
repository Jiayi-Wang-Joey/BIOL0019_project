...
nuclei$exp_eve2 <- as.numeric(nuclei$id %in% eve2$id)
...
model1 <- glm(exp_eve2 ~ bcdP__3 + hbP__3 + gtP__3 + KrP__3, data = train, family = binomial)
summary(model1)
...
model_mut <- glm(exp_eve2 ~ bcdP__3 + hbP_mut + gtP__3 + KrP__3, data = train, family = binomial)
...
```{r}
quadraticModel1 <- glm(train$exp_eve2 ~ bcdP__3 + hbP__3 + gtP__3 + KrP__3 + bcdP2, data = train, family = binomial)
...
```{r}
Qmodel_mut <- glm(exp_eve2 ~ bcdP__3 + hbP_mut + gtP__3 + KrP__3, data = train, family = binomial)
...
for(i in 1:23751){
  modelx <- glm(substitute(exp_eve2 ~ x + y + z + q,list(x = as.name(combinations[i,1]), y = as.name(combinations[i,2]), z = as.name(combinations[i,3]), q = as.name(combinations[i,4]))),
                data = train, family = binomial)
  predictedx <- predict(modelx,nuclei,type = "response")
  bi_predictedx <- as.numeric(predictedx > 0.5)
  mccrx <- mccr(nuclei$exp_eve2,bi_predictedx)
...
for(i in 1:23751){
  if("bcdP__3" %in% combinations[i,]){
    Qmodelx <- glm(substitute(exp_eve2 ~ x + y + z + q + bcdP2,list(x = as.name(combinations[i,1]), y = as.name(combinations[i,2]), z = as.name(combinations[i,3]), q = as.name(combinations[i,4]))),
                  data = train, family = binomial)
    Qpredictedx <- predict(Qmodelx,nuclei,type = "response")
    Qbi_predictedx <- as.numeric(Qpredictedx > 0.5)
    Qmccrx <- mccr(nuclei$exp_eve2,Qbi_predictedx)
    all_Qlogistic_model[i,1] <- Qmccrx
  }else{
    Qmodelx <- glm(substitute(exp_eve2 ~ x + y + z + q,list(x = as.name(combinations[i,1]), y = as.name(combinations[i,2]), z = as.name(combinations[i,3]), q = as.name(combinations[i,4]))),
                  data = train, family = binomial)
    Qpredictedx <- predict(Qmodelx,nuclei,type = "response")
    Qbi_predictedx <- as.numeric(Qpredictedx > 0.5)
    Qmccrx <- mccr(nuclei$exp_eve2,Qbi_predictedx)
...
space_between_12 <- subset(diffSub2, x__3 > (-95) & x__3 < -80 & z__3 < 0 | x__3 > -80 & x__3 < -60 | x__3 > -60 & x__3 < -50 & z__3 > 0)
space_between_34 <- subset(diffSub2, x__3 > -14 & x__3 < 20)
space_between_56 <- subset(diffSub2, x__3 > 50 & x__3 < 80 | x__3 > 80 & x__3 < 92 & z__3 < 0)
space_between_67 <- subset(diffSub2, x__3 > 80 & x__3 < 90 & z__3 > 0 | x__3 > 90 & x__3 < 110 | x__3 > 110 & x__3 < 135 & z__3 < 0)

test_dataset <- subset(nuclei,!(id %in% space_between_12$id) & !(id %in% space_between_34$id) &!(id %in% space_between_56$id) & !(id %in% space_between_67$id))
